import { isInteger, type DeepPartial, type ViewOffsets } from '../../utils';
...
export function addOffset(date: Date, viewOffsets: DeepPartial<ViewOffsets>): Date {
	...
	if (
		!viewOffsets ||
		typeof viewOffsets !== 'object' ||
		Object.getPrototypeOf(viewOffsets) !== Object.prototype
	) {
	...
	for (const [key, offset] of Object.entries(viewOffsets)) {
		const strategy = dateOffsetStrategies[key as keyof ViewOffsets];
